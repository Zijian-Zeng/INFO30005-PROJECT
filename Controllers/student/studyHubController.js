...
const getAll = async (req, res, next) => {
    try {
        ...
        for (hubId of subject.hubs) {
            ...
            if (studyHub) {
                const creator = await studentModel.findById(studyHub.creator);
                const students = [];
                let registered = false;

                for (each of studyHub.studentRegistered) {
                    const student = await studentModel.findById(each);

                    if (each == req.user.id) registered = true;
                    students.push({
                ...
                hubsInfo.push({
                    id: hubId,
                    subjectCode: studyHub.subjectCode,
                    startDate: studyHub.startDate,
                    endDate: studyHub.endDate,
                    location: studyHub.location,
                    summary: studyHub.summary,
                    creator: {
                    ...
                    students: students,
                    duration: studyHub.duration,
                    registered: registered,
                    day: studyHub.day,
                });
            } else {
                //Determine if the studyHub has been deleted.
                invalidIds.push(hubId);
            }
        }
        //Delete all those invalid studyHubs id.
        for (invalidId of invalidIds) {
            subject.hubs = subject.hubs.filter((each) => each != invalidId);
        }
        subject.save();

        //Response the studyHubs in the subject.
        if (hubsInfo == 0) {
            res.status(201).json({
                success: true,
